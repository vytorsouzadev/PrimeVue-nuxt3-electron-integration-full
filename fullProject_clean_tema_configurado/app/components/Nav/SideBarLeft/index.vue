<template>
    <div id="app" class="flex flex-col h-screen  transition-all duration-700 ease-in-out" :class="[sidebarStore.isOpen ? 'w-full' : 'w-16']">
        <!--header with toggle button-->
        <div class="px-1 py-2 overflow-y-auto flex flex-row justify-between items-center">
            <div class="w-1/4 px-1">
                <Button style="padding: 10px 20px!important;" size="large" id="bt-side-menu" icon="pi pi-bars" aria-label="Toggle Menu" @click="sidebarStore.toggle" />
            </div>
            <div class="w-3/4 text-right mr-1" v-if="sidebarStore.isOpen">
                <!-- <h2 class="text-lg font-medium">Menu Drawer</h2> -->
                <router-link to="/config/theme">
                    <Button icon="pi pi-home" aria-label="Home" label="Configurar Tema" />
                </router-link>
            </div>
        </div>
        ...
        <PanelMenu :model="items" @update:expandedKeys="handleExpandedKeysChange">
            <template #item="{ item, active }" >
                <a v-ripple class="flex items-center px-4 py-2 cursor-pointer group my-1 mr-2" 
